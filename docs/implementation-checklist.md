...
- [x] Определить минимальный retrieval contract (chunk metadata, top-k, citations).
- [x] Добавить заглушку индекса + интерфейс retriever.
- [x] Документировать ожидаемое поведение в `docs/spec.md` (без кода, на уровне контракта).
...
- [x] Подтвердить минимальный набор экранов/состояний для MVP.
- [x] Составить короткий список missing backend capabilities (если есть).
- [x] Закрыть критичные backend-блокеры app shell минимальными безопасными изменениями.
- **DoD:** есть чёткая граница «что уже можно собирать во фронте сегодня». ✅ Закрыто (2026-02-23).
