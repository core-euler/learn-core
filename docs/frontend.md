...
- Цель на текущем этапе: **рабочий макет**, без дизайн-полировки.
- Базовый паттерн интерфейса: как у ChatGPT/Claude — основной фокус на чате.
- Левая колонка используется не для «истории тем», а для **структуры курса** (модули/уроки).
- Визуальная сложность минимальная: нейтральная тема, читаемость, приоритет функциональности.
...
- Глобальный shell: левая колонка содержит структуру курса (модули/уроки), правая часть — рабочая зона.
- В lesson workspace основной блок — режимный чат (Lecture/Exam/Consultant), как primary interaction area.
- Материал урока показывается как secondary block ниже чата (без декоративной вёрстки).
- В шапке workspace: прогресс текущего модуля (завершено X из Y).
