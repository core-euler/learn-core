...
- [ ] Добавить валидатор структуры контента (на старте или отдельной командой).
- [ ] Добавить тест на битый/неполный контент.
- **DoD:** контент грузится предсказуемо, ошибки контента диагностируемы заранее.

### 2.3 Docker/ops polish
- [ ] Проверить healthcheck для backend/postgres.
- [ ] Зафиксировать `.env.example` с обязательными ключами.
- [ ] Документировать локальный и staging runbook.
- **DoD:** новый разработчик поднимает стенд по README без догадок.
